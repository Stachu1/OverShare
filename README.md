...
3. The zip is cut into 20 MB pieces. Each one is encrypted with **AES-256-GCM** and becomes an attachment named `<id>.<n>`; they're posted 10 to a message (Discord's attachment limit). Only about one message's worth of pieces (up to 200 MB) is in memory at a time, whatever the file size.
...
- Memory use stays about the same whatever the file size (a few hundred MB at most), because files are read, compressed and encrypted a piece at a time and uploaded 10 pieces at a time.
